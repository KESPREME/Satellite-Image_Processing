...
# Cache models with optimized settings
@st.cache_resource
def load_models():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    torch.backends.cuda.matmul.allow_tf32 = True  # Enable TensorFloat-32
    
    # Load optimized generation model (50% faster than SD1.5)
        ...
        try:
            ...
            if st.button("Analyze Land Cover", type="primary"):
                with st.spinner(f"Processing (est. 5-10s on {device.upper()})..."):
                    ...
                    colored_mask = color_palette[seg_map]
...
# System Info Footer
st.divider()
st.caption(f"Running on: {device.upper()} | Torch: {torch.__version__} | Streamlit: {st.__version__}")
